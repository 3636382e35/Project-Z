...
// if (text_part[0] == commands[command_1.spawn]) {
// 	#region CREATE
...
if (text_part[0] == commands[command_1.spawn]) {
	...
	var is_x_random = false;
	var is_y_random = false;
    ...
    if (parse_parts == 1) { 
        temp_post_message = "We need to know what object we are making";
        event_user(1);
    } else {
        if (!object_exists(object_to_try_and_make)) { 
            temp_post_message = "Object " + string(text_part[1]) + " doesn't exist";
            event_user(1);
        } else {
            create_obj = object_to_try_and_make;

            if (parse_parts >= 3) {
                if (text_part[2] == "mouse_x") {
                    create_x = mouse_x;
                } else if (text_part[2] == "mouse_y") {
                    create_x = mouse_y;
                } else if (text_part[2] == "random") {
                    is_x_random = true;
                } else {
                    if (text_part[2] != "" and text_part[2] != " ") {
                        create_x = real(text_part[2]);
            ...
            if (parse_parts >= 4) {
                if (text_part[3] == "mouse_x") {
                    create_y = mouse_x;
                } else if (text_part[3] == "mouse_y") {
                    create_y = mouse_y;
                } else if (text_part[3] == "random") {
                    is_y_random = true;
                } else {
                    if (text_part[3] != "" and text_part[3] != " ") {
                        create_y = real(text_part[3]);
            ...
            if(parse_parts >= 5){
            	if (text_part[4] != "" and text_part[4] != " ") {
            		var times = real(text_part[4]);
            		repeat(times){
            			randomize();
            			if is_x_random create_x = random_range(0, room_width);
            			if is_y_random create_y = random_range(0, room_height);
            			instance_create_depth(create_x, create_y, -10000, create_obj);
            		}
            	}
            } else {
            	instance_create_layer(create_x, create_y, "Entities", create_obj);
            }
	...
	#endregion
} else if (text_part[0] == commands[command_1.restart]){
		game_restart();
