...
if instance_exists(o_Player){
	if o_Player.DEBUG_MODE {
		if instance_exists(self){
			...
			// if instance_exists(o_Player){
			// 	if self.on_air draw_line_width_colour(x,y-5 , o_Player.x, o_Player.y-5,1,c_yellow, c_yellow);
			// 	else draw_line_width_colour(x,y-5 , o_Player.x, o_Player.y-5,1,c_red, c_red);
		}
	}
...
if o_Player.HP_ENABLED {
	...
	//draw_rectangle_color(x-x_offset,  y-height, (x-x_offset) + hp_width, y-(height-2), c_red, c_red, c_red, c_red, false);
	...
	//     // Draw the line on top of the object with interpolated outline thickness
	//   	//draw_line_width_color(o_Player.x+start_x, o_Player.y +start_y, end_x, end_y, 2, c_white, c_white);
	//   	//draw_line_width_color(start_x, start_y, end_x, end_y, 3, c_red, c_red);

	//    	// randomize();
	//     //var outline_color = choose(c_black,c_red,c_fuchsia,c_yellow,c_green,c_aqua,c_purple);
	...
	//         if (id == o_death_mob.id) { // Ensure it's not checking against itself
	...
	//TODO: add floating damage text shits (optional)
